...
#error Unsupported platform
  ...
  template <typename T> ErrorOr<T> getSymbol(const char *name) {
    ...
    const char *err = dlerror();
    if (err) {
      return error(ErrorCode::InternalError, err);
    }
#else
#error Unsupported platform
#endif
...
#error Unsupported platform
  ...
  /// Returns the native handle to the loaded library.
  /// Use with caution - the handle type differs between platforms.
#ifdef FUSILLI_PLATFORM_WINDOWS
  HMODULE getNativeHandle() const { return handle_; }
#elif FUSILLI_PLATFORM_LINUX
