...
inline ErrorOr<std::shared_ptr<Buffer>>
allocateWorkspace(const Handle &handle, std::optional<size_t> workspaceSize) {
...
inline ErrorObject checkMlirIndentation(const std::string &mlir) {
  ...
  while (std::getline(stream, line)) {
    ...
    // Ops inside @main must be indented at least 4 spaces. Deeper
    // indents are allowed for nested region bodies (e.g. linalg.generic).
