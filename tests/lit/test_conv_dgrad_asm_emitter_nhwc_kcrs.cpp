...
// TORCH-CHECK:     func.func @main(%result_: !torch.tensor<[16,64,32,128],f32>, %arg0_dy: !torch.vtensor<[16,64,32,256],f32>, %arg1_w: !torch.vtensor<[256,128,1,1],f32>) attributes {torch.assume_strict_symbolic_shapes} {
...
// TORCH-CHECK:       %groups_conv_dgrad = torch.constant.int 1
...
// TORCH-CHECK:       %arg1_w_conv_dgrad_perm = torch.aten.permute %arg1_w, %permute_W_conv_dgrad : !torch.vtensor<[256,128,1,1],f32>, !torch.list<int> -> !torch.vtensor<[256,128,1,1],f32>
...
// TORCH-CHECK:       %result_conv_dgrad_perm, %grad_weight_conv_dgrad, %grad_bias_conv_dgrad = torch.aten.convolution_backward %arg0_dy_conv_dgrad_perm, %empty_x_conv_dgrad, %arg1_w_conv_dgrad_perm, %bias_conv_dgrad, %stride_conv_dgrad, %padding_conv_dgrad, %dilation_conv_dgrad, %transposed_conv_dgrad, %output_padding_conv_dgrad, %groups_conv_dgrad, %output_mask_conv_dgrad : !torch.vtensor<[16,256,64,32],f32>, !torch.vtensor<[16,128,64,32],f32>, !torch.vtensor<[256,128,1,1],f32>, !torch.none, !torch.list<int>, !torch.list<int>, !torch.list<int>, !torch.bool, !torch.list<int>, !torch.int, !torch.list<bool> -> !torch.vtensor<[16,128,64,32],f32>, !torch.none, !torch.none
...
// LINALG-CHECK:      %[[BUF2:.+]] = hal.tensor.import wait(%[[ARG3]]) => %[[ARG2]] : !hal.buffer_view -> tensor<256x128x1x1xf32>
...
// LINALG-CHECK:      %[[EMPTY:.+]] = tensor.empty() : tensor<16x128x64x32xf32>
// LINALG-CHECK:      %[[FILLED:.+]] = linalg.fill ins(%[[CST]] : f32) outs(%[[EMPTY]] : tensor<16x128x64x32xf32>) -> tensor<16x128x64x32xf32>
// LINALG-CHECK:      %[[GEN:.+]] = linalg.generic
// LINALG-CHECK-SAME:   ins(%[[BUF1_T]], %[[BUF2]] : tensor<16x256x64x32xf32>, tensor<256x128x1x1xf32>)
// LINALG-CHECK-SAME:   outs(%[[FILLED]] : tensor<16x128x64x32xf32>)
// LINALG-CHECK:      %[[EOUT:.+]] = tensor.empty() : tensor<16x64x32x128xf32>
// LINALG-CHECK:      %[[OUT_T:.+]] = linalg.transpose ins(%[[GEN]] : tensor<16x128x64x32xf32>) outs(%[[EOUT]] : tensor<16x64x32x128xf32>) permutation = [0, 2, 3, 1]
...
int main(int argc, char **argv) {
  ...
  auto status = testConvDgradAsmEmitterDyNhwcDxNhwc(mode);
  if (isError(status)) {
