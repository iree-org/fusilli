...
// TORCH-CHECK:       %permute_x_val_2_rmsnorm_infer = torch.constant.int 2
// TORCH-CHECK:       %permute_x_val_3_rmsnorm_infer = torch.constant.int 3
...
// TORCH-CHECK:       %arg0_x_rmsnorm_infer_perm = torch.aten.permute %arg0_x, %permute_x_rmsnorm_infer : !torch.vtensor<[16,128,64,32],f32>, !torch.list<int> -> !torch.vtensor<[16,128,64,32],f32>
// TORCH-CHECK:       %none_scale_rmsnorm_infer = torch.constant.none
// TORCH-CHECK:       %result_rmsnorm_infer_perm = torch.aten.rms_norm %arg0_x_rmsnorm_infer_perm, %normalized_shape_rmsnorm_infer, %none_scale_rmsnorm_infer, %eps_rmsnorm_infer : !torch.vtensor<[16,128,64,32],f32>, !torch.list<int>, !torch.none, !torch.float -> !torch.vtensor<[16,128,64,32],f32>
// TORCH-CHECK:       %permute_y_val_0_rmsnorm_infer = torch.constant.int 0
// TORCH-CHECK:       %permute_y_val_1_rmsnorm_infer = torch.constant.int 1
// TORCH-CHECK:       %permute_y_val_2_rmsnorm_infer = torch.constant.int 2
// TORCH-CHECK:       %permute_y_val_3_rmsnorm_infer = torch.constant.int 3
...
// TORCH-CHECK:       %result = torch.aten.permute %result_rmsnorm_infer_perm, %permute_y_rmsnorm_infer : !torch.vtensor<[16,128,64,32],f32>, !torch.list<int> -> !torch.vtensor<[16,128,64,32],f32>
...
static ErrorObject testRmsnormInferAsmEmitterNchw(const std::string &mode) {
  int64_t n = 16, c = 128, h = 64, w = 32;
  auto graph = std::make_shared<Graph>();
  graph->setName("rmsnorm_infer_asm_emitter_nchw");
...
int main(int argc, char **argv) {
  ...
  auto status = testRmsnormInferAsmEmitterNchw(mode);
  if (isError(status)) {
