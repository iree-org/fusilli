...
// TORCH-CHECK:     func.func @main(%result_: !torch.tensor<[16,64,1,32,256],f32>, %arg0_image: !torch.vtensor<[16,2,64,32,128],f32>, %arg1_filter: !torch.vtensor<[256,2,128,1,1],f32>) attributes {torch.assume_strict_symbolic_shapes} {
...
// TORCH-CHECK:       %arg0_image_conv_fprop_perm = torch.aten.permute %arg0_image, %permute_X_conv_fprop : !torch.vtensor<[16,2,64,32,128],f32>, !torch.list<int> -> !torch.vtensor<[16,128,2,64,32],f32>
...
// TORCH-CHECK:       %arg1_filter_conv_fprop_perm = torch.aten.permute %arg1_filter, %permute_W_conv_fprop : !torch.vtensor<[256,2,128,1,1],f32>, !torch.list<int> -> !torch.vtensor<[256,128,2,1,1],f32>
// TORCH-CHECK:       %result_conv_fprop_perm = torch.aten.convolution %arg0_image_conv_fprop_perm, %arg1_filter_conv_fprop_perm, %bias_conv_fprop, %stride_conv_fprop, %padding_conv_fprop, %dilation_conv_fprop, %transposed_conv_fprop, %output_padding_conv_fprop, %groups_conv_fprop : !torch.vtensor<[16,128,2,64,32],f32>, !torch.vtensor<[256,128,2,1,1],f32>, !torch.none, !torch.list<int>, !torch.list<int>, !torch.list<int>, !torch.bool, !torch.list<int>, !torch.int -> !torch.vtensor<[16,256,1,64,32],f32>
...
// TORCH-CHECK:       %result = torch.aten.permute %result_conv_fprop_perm, %permute_Y_conv_fprop : !torch.vtensor<[16,256,1,64,32],f32>, !torch.list<int> -> !torch.vtensor<[16,64,1,32,256],f32>
...
// LINALG-CHECK:      %[[BUF2:.+]] = hal.tensor.import wait(%{{.+}}) => %[[ARG2]] : !hal.buffer_view -> tensor<256x2x128x1x1xf32>
...
// LINALG-CHECK:      %[[BUF2T:.+]] = linalg.transpose ins(%[[BUF2]] : tensor<256x2x128x1x1xf32>) outs(%{{.+}} : tensor<256x128x2x1x1xf32>) permutation = [0, 2, 1, 3, 4]
...
// LINALG-CHECK:      %[[OUTT:.+]] = linalg.transpose ins(%[[OUT]] : tensor<16x256x1x64x32xf32>) outs(%{{.+}} : tensor<16x64x1x32x256xf32>) permutation = [0, 3, 2, 4, 1]
// LINALG-CHECK:      %{{.+}} = hal.tensor.alias wait(%{{.+}}) => %[[OUTT]] : tensor<16x64x1x32x256xf32> to %[[ARG0]] : !hal.buffer_view
//
