...
// Verifies that an RMSNorm with a partial trailing-suffix scale shape
// (scale=[1,1,H,W] for x=[N,C,H,W]) emits torch.aten.rms_norm with
// normalized_shape=[H, W] (only the trailing matching dims of x), not the
// whole non-batch shape.
...
static ErrorObject
testRmsnormInferAsmEmitterPartialSuffixHw(const std::string &mode) {
  int64_t n = 16, c = 128, h = 64, w = 32;
  auto graph = std::make_shared<Graph>();
  graph->setName("rmsnorm_infer_asm_emitter_partial_suffix_hw");
  ...
  // scale=[1, 1, H, W]: trailing match is the last 2 dims of x → expect
  // normalized_shape=[H, W].
  auto scaleT = graph->tensor(TensorAttr()
                                  .setName("arg0_scale")
                                  .setDim({1, 1, h, w})
                                  .setStride({h * w, h * w, w, 1}));
...
int main(int argc, char **argv) {
  ...
  auto status = testRmsnormInferAsmEmitterPartialSuffixHw(mode);
  if (isError(status)) {
