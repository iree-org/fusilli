...
// LINALG-CHECK:      %[[INPUT:.+]] = hal.tensor.import wait(%{{.+}}) => %[[ARG1]] : !hal.buffer_view -> tensor<16x64x32x128xf32>
// LINALG-CHECK:      %[[SCALE:.+]] = hal.tensor.import wait(%{{.+}}) => %[[ARG2]] : !hal.buffer_view -> tensor<1x128x64x32xf32>
// LINALG-CHECK:      %[[BIAS:.+]] = hal.tensor.import wait(%{{.+}}) => %[[ARG3]] : !hal.buffer_view -> tensor<1x128x64x32xf32>
...
// LINALG-CHECK:      %{{.+}} = hal.tensor.alias wait(%{{.+}}) => %[[OUT]] : tensor<16x64x32x128xf32> to %[[ARG0]] : !hal.buffer_view
//
...
testLayernormInferAsmEmitterScaleBiasNhwc(const std::string &mode) {
  ...
  graph->setName("layernorm_infer_asm_emitter_scale_bias_nhwc");
  ...
  auto [yT, mT, vT] = graph->layernorm(xT, sT, bT, epsilonT, layernormAttr);
