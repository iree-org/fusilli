...
// TORCH-CHECK:       %y = torch.aten.permute %y_perm, %permute_y_layernorm_train : !torch.vtensor<[16,128,64,32],f32>, !torch.list<int> -> !torch.vtensor<[16,128,64,32],f32>
...
// LINALG-CHECK:      %[[INPUT:.+]] = hal.tensor.import wait(%{{.+}}) => %[[ARG3]] : !hal.buffer_view -> tensor<16x128x64x32xf32>
...
// LINALG-CHECK:      %{{.+}} = hal.tensor.alias wait(%{{.+}}) => %[[OUT]] : tensor<16x128x64x32xf32> to %[[ARG2]] : !hal.buffer_view
// LINALG-CHECK:      %{{.+}}:3 = hal.tensor.barrier join(%{{.+}}, %{{.+}}, %{{.+}} : tensor<16x1x1x1xf32>, tensor<16x1x1x1xf32>, tensor<16x128x64x32xf32>) => %{{.+}} : !hal.fence
//
// AMDGPU-STATS-CHECK: "dispatch-count": 4
...
static ErrorObject testLayernormTrainAsmEmitterNchw(const std::string &mode) {
  ...
  auto [yT, mT, vT] =
      graph->layernorm(xT, nullptr, nullptr, epsilonT, layernormAttr);
