...
// clang-format off
//
// GQA with different K and V head counts: Q=[1,8,64,64], K=[1,4,64,64], V=[1,2,64,64].
//
// TORCH-CHECK:   module @module {
// TORCH-CHECK:     func.func @main(%sdpa_O_: !torch.tensor<[1,8,64,64],f16>, %k: !torch.vtensor<[1,4,64,64],f16>, %q: !torch.vtensor<[1,8,64,64],f16>, %v: !torch.vtensor<[1,2,64,64],f16>) attributes {torch.assume_strict_symbolic_shapes} {
...
// TORCH-CHECK:       %k_sdpa_perm = torch.aten.permute %k, %permute_K_sdpa : !torch.vtensor<[1,4,64,64],f16>, !torch.list<int> -> !torch.vtensor<[1,4,64,64],f16>
...
static ErrorObject testSdpaAsmEmitterGqaHkNeHv(const std::string &mode) {
  auto graph = std::make_shared<Graph>();
  graph->setName("sdpa_asm_emitter_gqa_hk_ne_hv").setIODataType(DataType::Half);

  std::vector<int64_t> qDim = {1, 8, 64, 64};
  auto qStride =
      generateStrideFromDim(qDim, getContiguousStrideOrder(qDim.size()));

  std::vector<int64_t> kDim = {1, 4, 64, 64};
  auto kStride =
      generateStrideFromDim(kDim, getContiguousStrideOrder(kDim.size()));

  std::vector<int64_t> vDim = {1, 2, 64, 64};
  auto vStride =
...
int main(int argc, char **argv) {
  ...
  auto status = testSdpaAsmEmitterGqaHkNeHv(mode);
  if (isError(status)) {
