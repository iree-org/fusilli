...
// TORCH-CHECK:     func.func @main(%inv_rms_: !torch.tensor<[16,1,1,1],f32>, %y_: !torch.tensor<[16,128,64,32],f32>, %arg0_x: !torch.vtensor<[16,128,64,32],f32>) attributes {torch.assume_strict_symbolic_shapes} {
...
// TORCH-CHECK:       %permute_x_val_1_rmsnorm_train = torch.constant.int 1
// TORCH-CHECK:       %permute_x_val_2_rmsnorm_train = torch.constant.int 2
// TORCH-CHECK:       %permute_x_val_3_rmsnorm_train = torch.constant.int 3
...
// TORCH-CHECK:       %arg0_x_rmsnorm_train_perm = torch.aten.permute %arg0_x, %permute_x_rmsnorm_train : !torch.vtensor<[16,128,64,32],f32>, !torch.list<int> -> !torch.vtensor<[16,128,64,32],f32>
// TORCH-CHECK:       %none_scale_rmsnorm_train = torch.constant.none
// TORCH-CHECK:       %y_rmsnorm_train_perm, %inv_rms_rmsnorm_train_perm = torch.aten.native_rms_norm %arg0_x_rmsnorm_train_perm, %normalized_shape_rmsnorm_train, %none_scale_rmsnorm_train, %eps_rmsnorm_train : !torch.vtensor<[16,128,64,32],f32>, !torch.list<int>, !torch.none, !torch.float -> !torch.vtensor<[16,128,64,32],f32>, !torch.vtensor<[16,1,1,1],f32>
// TORCH-CHECK:       %permute_y_val_0_rmsnorm_train = torch.constant.int 0
// TORCH-CHECK:       %permute_y_val_1_rmsnorm_train = torch.constant.int 1
// TORCH-CHECK:       %permute_y_val_2_rmsnorm_train = torch.constant.int 2
// TORCH-CHECK:       %permute_y_val_3_rmsnorm_train = torch.constant.int 3
...
// TORCH-CHECK:       %y = torch.aten.permute %y_rmsnorm_train_perm, %permute_y_rmsnorm_train : !torch.vtensor<[16,128,64,32],f32>, !torch.list<int> -> !torch.vtensor<[16,128,64,32],f32>
...
static ErrorObject testRmsnormTrainAsmEmitterNchw(const std::string &mode) {
  int64_t n = 16, c = 128, h = 64, w = 32;
  auto graph = std::make_shared<Graph>();
  graph->setName("rmsnorm_train_asm_emitter_nchw");
...
int main(int argc, char **argv) {
  ...
  auto status = testRmsnormTrainAsmEmitterNchw(mode);
  if (isError(status)) {
